...
class Entity(object):
    ...
    def getvals(self, keys=None):
        if keys is None:
            keys = Schema.attrs(self.name)
        return dict(zip(keys, map(lambda key: self.getval(key), keys)))
...
class RouterNode(Entity):
    instance = 0
    def __init__(self, f, t, links, nodes, hopper):
        super(RouterNode, self).__init__("router.node")
        self.hopper = hopper
        self.init(f, t, links, nodes)

    def init(self, f, t, links, nodes):
        RouterNode.instance += 1
        self.setval("name", "router.node/" + t)
        self.setval("nextHop", "(self)" if f == t else self.hopper.get(f, t, links, nodes))
...
class Listener(Entity):
    ...
    def init(self, port):
        ...
        self.setval("host", "0.0.0.0")
        self.setval("identity", "listener/0.0.0.0:" + str(port))
        self.setval("name", self.getval("identity"))
        self.setval("maxFrameSize", 16384)
