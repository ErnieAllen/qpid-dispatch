...
//QDR.log.debug("queueDepth requested " + qdepth + "(" + self.correlator.depth() + ")")
      ...
      sendMethod: function(operation, props, callback) {
        setTimeout(function () {
          ...
          //$.get( "http://localhost:8000/?" + operation, props, function(response, status) {
          //  callback(nodeId, entity, response, {})
          //}, "json")
        }, 1)
      },
      ...
      testConnect: function (options, timeout, callback) {
        ...
        try {
          ...
          QDR.log.info("exception caught on test connect " + e)
      ...
      connect: function(options) {
        ...
        if (!self.connected) {
          ...
          QDR.log.info("****** calling rhea.connect ********")
          if (!options.connection) {
            QDR.log.info("rhea.connect was not passed an existing connection")
            options.reconnect = true
            self.connected = true
            self.onSubscription();
            //self.testConnect(options, 5000, connectionCallback)
          } else {
            QDR.log.info("rhea.connect WAS passed an existing connection")
