...
var QDR = (function(QDR) {
  ...
  QDR.module.controller("QDR.TopologyController", ['$scope', '$rootScope', 'QDRService', '$location', '$timeout', '$uibModal',
    function($scope, $rootScope, QDRService, $location, $timeout, $uibModal) {
      ...
      var schema;
      QDRService.getSchema( function (results) {
          schema = QDRService.schema
    ...
    "children": [
      ...
      },
      {
        ...
        var tmargin = {top: 20, right: 120, bottom: 20, left: 120},
          twidth = 660 - tmargin.right - tmargin.left,
        ...
        function click(d) {
          $timeout( (function () {
            ...
            // remove all selected classes from all nodes (except this one)
        ...
        function mouseover(d) {
          $timeout( (function () {
            d3.select(this).classed("hovered",
              function (d) {
                return d3.select('g.selected').empty() ? "hovered" : ""
              })
      ...
      $scope.formCancel = function () {
        $timeout( function () {
          ...
          $scope.formData = angular.copy($scope.shadowData)
          $scope.formMode = 'view'
          $scope.showForm = 'none'
        })
      }
      ...
      $scope.validateName = function () {
        if ($scope.formData.name === '')
          return
        $timeout( function () {
          $scope.formData.duplicate = false
          $scope.editVhost.$setValidity('duplicate', true);
          // check for duplicate names
          var foundName = false
          for (var i=0; i<root.children.length; i++) {
            if (root.children[i].name === $scope.formData.name) {
              if (foundName) {
                $scope.formData.duplicate = true
                $scope.editVhost.$setValidity('duplicate', false);
              }
              foundName = true
            }
          }
